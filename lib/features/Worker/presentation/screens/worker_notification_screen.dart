...
class WorkerNotificationScreen extends StatefulWidget {
  ...
  @override
  State<WorkerNotificationScreen> createState() =>
      _WorkerNotificationScreenState();
}

class _WorkerNotificationScreenState extends State<WorkerNotificationScreen>
  ...
  Future<void> _markNotificationsAsSeen() async {
    ...
    if (workerManager.newNotificationCount > 0) {
      _hasTriggeredSeenUpdate = true;

      try {
        await workerManager.markAllNotificationsAsSeenNew();
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context) {
    return AppBar(
      ...
      actions: [
        Consumer<WorkerManagerProvider>(
          builder: (context, provider, child) {
            final hasNotifications = provider.notifications.isNotEmpty;
            final hasUnreadNotifications = provider.totalUnreadCount > 0;
  ...
  Future<void> _markAllAsRead(
      BuildContext context, WorkerManagerProvider provider) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(AppLocalizations.of(context)!.failedToMarkAsRead),
              ...
              return SlideTransition(
                ...
                child: FadeTransition(
                  ...
                  child: _buildNotificationCard(
                      context, notification, index, provider),
                ),
              );
            },
          );
  ...
  Widget _buildNotificationCard(
    ...
    final timeString =
        DateFormat('MMM dd, h:mm a').format(notification.createdAt.toLocal());
  ...
  Future<void> _handleNotificationTap(
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Notification from ${notification.userName}'),
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(10),
            ),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text(AppLocalizations.of(context)!.errorOpeningNotification),
            backgroundColor: AppColors.errorDark,
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(10),
